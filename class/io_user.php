...
<?php
	...
	public static function user_ldap_add($user_id) {
		...
		$ldapConn->addUser(get_post_meta($user_id, 'first_user', true), get_post_meta($user_id, 'last_user', true));
		$ldapConn->setUserAttribute(get_userdata($user_id)->user_login, "user_art", get_post_meta($user_id, 'user_art', true));
		
		if(get_user_meta($user_id, 'user_art', true) == 'Basisgruppe' || get_user_meta($user_id, 'user_art', true) == 'Landesverband') {
			$ldapConn->setUserAttribute(get_userdata($user_id)->user_login, "user_land", get_post_meta($user_id, 'land', true));
		}
		
		if(get_user_meta($user_id, 'user_art', true) == 'Basisgruppe') {
			$ldapConn->setUserAttribute(get_userdata($user_id)->user_login, "user_ort", get_post_meta($user_id, 'ort', true));
		}
	}
}
